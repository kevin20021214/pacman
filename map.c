...
static void draw_transportation_bean(Map* M, const int row, const int col);
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				...
				case 'P':
					...
					break;
				case 'T':
					draw_transportation_bean(M, row, col);
					break;
				case '.':
					draw_bean(M, row, col);
					break;
...
static void draw_transportation_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(22, 118, 238));
}
static void draw_speed_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(20, 200, 38));
}
